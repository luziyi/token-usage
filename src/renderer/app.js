(function () {
  ...
  function modelIconSrc(modelId, provider, modelKey) {
    ...
    let icon = MODEL_ICONS[mk] || MODEL_ICONS[prov] || MODEL_ICONS[id] || null;
    if (!icon) {
      for (const [key, val] of Object.entries(MODEL_ICONS)) {
        if (id.includes(key) || prov.includes(key) || mk.includes(key)) { icon = val; break; }
      }
    }
  ...
  function render() {
    if (state.sessionDetail) {
      state.sessionDetail.loading ? renderSessionDetailLoading() : renderSessionDetail();
      return;
    }
    ...
    els.inputLabel.textContent = 'IN ' + formatNumber(aggregated.totals.totalInput);
    els.outputLabel.textContent = 'OUT ' + formatNumber(aggregated.totals.totalOutput);
    els.cacheReadLabel.textContent = 'CACHE ' + formatNumber(aggregated.totals.totalCacheRead + aggregated.totals.totalCacheWrite);
  ...
  function renderBreakdown(periodData, currency) {
    ...
    if (mode === 'model') {
      ...
      groups = Object.entries(dataSource).map(([key, d]) => ({
        ...
        provider: d.provider,
        label: key,
        sublabel: d.provider || getProviderLabel(key),
    ...
    if (groups.length === 0) {
      els.breakdown.innerHTML = '<div class="empty-state"><div class="empty-state-icon">&#9632;</div><div class="empty-state-text">暂无数据</div></div>';
      return;
    }
    ...
    for (const g of groups) {
      ...
      const iconPath = modelIconSrc(g.key, g.provider, g.modelKey);
  ...
  function renderSessionDetailLoading() {
    ...
    els.sessionDetail.innerHTML = '<div class="detail-header"><button class="detail-back">&#8249; 返回</button><span class="detail-info">加载中…</span></div>';
    els.sessionDetail.querySelector('.detail-back').addEventListener('click', closeSessionDetail);
  }
  ...
  function renderSessionDetail() {
    ...
    info.textContent = detail.exchanges ? detail.exchanges.length + ' 轮对话' : '';
    header.append(info);
    ...
    if (detail.summary) {
      ...
      els.cost.textContent = formatCost(detail.summary.totalCost || 0, state.settings?.currency);
    }

    const sorted = [...exchanges].sort((a, b) => (a.startedAt || '').localeCompare(b.startedAt || ''));
  ...
  function createExchangeNode(row, max) {
    ...
    exTitle.append(document.createTextNode(row.promptPreview || '(会话开始)'));
    ...
    wrap.querySelector('.detail-ex-value').textContent = formatNumber(row.tokens?.total || 0);
  ...
  function createTurnNode(turn, index) {
    ...
    const split = 'IN ' + formatNumber(tk.input || 0)
      + ' · OUT ' + formatNumber(tk.output || 0)
  ...
  function applySettingsToUI(s) {
    ...
    if (els.refreshInput) els.refreshInput.value = String(settings.refreshMs || 5000);
  }
  ...
  function toggleMenu() {
    ...
    for (const m of BREAKDOWN_MODES) {
      ...
      item.className = 'dropdown-item' + (m.key === state.breakdown ? ' active' : '');
  ...
  function handleDataPush(payload) {
    ...
    updateLiveDot(true);
    els.status.textContent = '更新于 ' + new Date(payload.at).toLocaleTimeString();
  ...
  function init() {
    ...
    els.refreshButton.addEventListener('click', () => {
      ...
      els.refreshButton.style.transition = 'transform 0.4s ease';
      api.refreshData().then((data) => {
        setTimeout(() => { els.refreshButton.style.transition = 'none'; els.refreshButton.style.transform = 'rotate(0deg)'; }, 400);
